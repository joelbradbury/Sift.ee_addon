...
class Cartthrob_core_ee extends Cartthrob_core
{
	...
	public function get_cart()
	{
		...
		$this->EE->load->library('cartthrob_session', $params);
		
		$cart_id = $this->EE->cartthrob_session->cart_id(); 
		
		$this->EE->load->model('cart_model');
		$cart = $this->EE->cart_model->read_cart($cart_id);
		...
		$cart['customer_info']['ip_address'] = $this->EE->input->ip_address();
		
		if (empty($cart['customer_info']['currency_code']))
		{
		...
		if ($this->EE->session->userdata('member_id') && $this->store->config('save_member_data'))
		{
			...
			{
				//@TODO test this
				$profile_edit_entry_id = $this->EE->profile_model->get_profile_id($this->EE->session->userdata('member_id'));
				...
				if ($member_data = $this->EE->cartthrob_entries_model->entry($profile_edit_entry_id))
				{
					...
					{
						if ($member_field = $this->store->config('member_'.$key.'_field'))
						{
							if (isset($member_data['field_id_'.$member_field]))
							{
								$cart['customer_info'][$key] = $member_data['field_id_'.$member_field];
							}
							else if ( ! is_numeric($member_field) && $this->EE->session->userdata($member_field))
							{
								$cart['customer_info'][$key] = $this->EE->session->userdata($member_field);
							}
						}
					}
	...
	public function save_cart()
	{
		...
		$id = $this->EE->cartthrob_session->cart_id(); 

		$this->EE->load->model('cart_model');
		$this->EE->cart_model->update_cart($id, $this->cart_array(), $this->EE->functions->fetch_current_uri());
		...
		foreach ($this->cart->items() as $row_id => $item)
		{
			...
			if ($item->sub_items())
			{
				foreach ($item->sub_items() as $sub_item)
				{
					...
					{
						$product->reduce_inventory($sub_item->quantity(), $sub_item->item_options());
					}
				}
			}
	...
	*/
	...
	public function cart_info()
	{
		...
		$subtotal_plus_tax = 0; 
		foreach ($this->cart->taxable_items() as $key => $item)
		{
			$item_price_plus_tax = $item->price() + ($this->EE->api_cartthrob_tax_plugins->tax_rate() * $item->price()); 
			$subtotal_plus_tax += $this->round( $item_price_plus_tax ) * $item->quantity();
 		...
 		return array(
			...
			'cart_subtotal_plus_tax' => $this->EE->number->format( $subtotal_plus_tax  ),
			'cart_subtotal:plus_tax' => $this->EE->number->format( $subtotal_plus_tax  ),
			'cart_tax' => $this->EE->number->format($this->cart->tax()),
			'cart_shipping' => $this->EE->number->format($this->cart->shipping()),
			'cart_shipping_plus_tax' => $this->EE->number->format( $this->cart->shipping() + ($this->EE->api_cartthrob_tax_plugins->tax_rate() * $this->cart->shipping() )),
			'cart_shipping:plus_tax' => $this->EE->number->format( $this->cart->shipping() + ($this->EE->api_cartthrob_tax_plugins->tax_rate() * $this->cart->shipping() )),
