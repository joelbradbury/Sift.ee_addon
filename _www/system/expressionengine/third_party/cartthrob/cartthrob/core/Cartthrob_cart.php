...
class Cartthrob_cart extends Cartthrob_child
{
	protected $items, $total, $subtotal, $shippable_subtotal, $taxable_subtotal,
		  ...
		  $coupon_codes, $order, $meta, $config;
	 ...
	 * @return string|float
	 */
	public function discount($clear_cache = FALSE)
	{
		if ($clear_cache === TRUE)
		{
			...
			else
			{
				...
				foreach ($this->coupon_codes() as $coupon_code)
				{
					...
					if ($this->core->validate_coupon_code($coupon_code) && ! empty($data['type']))
					{
						...
						{
							$cache += $this->core->round($plugin->get_discount());
						}
					}
				}
	...
	public function total($clear_cache = FALSE)
	{
		...
		if ($this->calculation_caching === FALSE || is_null($cache))
		{
			...
			{
				$cache = $this->subtotal() + $this->tax() + $this->shipping() - $this->discount();
	...
	public function subtotal($clear_cache = FALSE)
	{
		...
		if ($this->calculation_caching === FALSE || is_null($cache))
		{
			...
			else
			{
				...
				foreach ($this->items() as $item)
				{
					$cache += $item->price() * $item->quantity();
				}
			}
			...
			else
			{
				...
				if ($this->core->store->config('tax_plugin'))
				{
					...
					if (method_exists($plugin, 'get_tax'))
					{
						...
						{
							$tax += $this->core->round($plugin->get_tax($item->price())) * $item->quantity(); 
						}
						
						if ($this->core->cart->discount())
						{
							$tax -= $this->core->round($plugin->get_tax( $this->core->cart->discount() )); 
						}
						
						if ($plugin->tax_shipping())
						{
							$tax += $this->core->round($plugin->get_tax($this->core->cart->shipping()));
						}
