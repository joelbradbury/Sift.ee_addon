...
$lang = array(
	...
	// MODULES page
	//----------------------------------------
	'cartthrob_module_name'		=> 'CartThrob',
	...
	'member_login_options_description'	=>'When registering users during the checkout process, you must select auto-login if you wish to save the customer\'s address and billing data to EE member fields. If you do not choose auto-login, standard EE member activation settings will be honored. When using an offsite gateway like PayPal Standard, customer data may not be able to be stored, and the customer may not be automatically be logged in. If you require membership as well as the use of offsite gateways then it is recommended that you register users before they checkout. ',
	...
	'global_settings_login_description' => 'If set to "yes" customer will be not be able to checkout & the checkout_form will not be shown. If set to "no" no membership will be required for checkout.',
	...
	'global_settings_garbage_collection_cron_description' => get_instance()->functions->insert_action_ids('On high-traffic sites, the built-in session garbage collection (removing old sessions) can intermittently hinder front-end performance. You can work around this by setting this option to yes, which will disable the front-end from triggering garbage collection. You will then be resonsible for setting up a cron job to manage the garbage collection. This is your cron job url:<br><br><code>'.get_instance()->functions->fetch_site_index(0, 0).QUERY_MARKER.'ACT='.get_instance()->functions->fetch_action_id('Cartthrob_mcp', 'garbage_collection').'</code><br><br>For instance, using curl you could set up your cron job like this: <br><br><code>curl --silent "'.get_instance()->functions->fetch_site_index(0, 0).QUERY_MARKER.'ACT='.get_instance()->functions->fetch_action_id('Cartthrob_mcp', 'garbage_collection').'" 2>&1 /dev/null</code>'),
	...
	'orders_last_four_digits_instructions' => 'Storing entire credit card numbers is not a safe idea. Storing the last 4 digits of a credit card for future reference is common practice however. See <a href="https://www.pcisecuritystandards.org/">PCI Standards</a> for more details.',
	...
	'payment_security_header' => 'Payment Security Settings',
