...
class Cartthrob_cardsave_server extends Cartthrob_payment_gateway
{
	...
	public function process_payment($credit_card_number)
	{
		...
		$string_to_hash = substr($data, 0, -1);
	...
	public function result($post)
	{
		...
		if ( empty($post['HashDigest']) || !isset($post['StatusCode']))
		{
			$resp['error_message']	= $this->lang('cardave_server_data_not_specified'); 
			$this->gateway_order_update($resp, $this->order('order_id'), NULL);
			echo "StatusCode=30&Message=".	$resp['error_message']; 
			exit; 
		}
		
		$status = intval($post["StatusCode"]); 

		$expected_array_keys = array(
			'PostCode',
			'State',
			'CountryCode',
			'City',
		...
		foreach($expected_array_keys as $item)
		{
			if (!in_array($item, $post))
			{
				if (isset($post['Message']))
				{
					$resp['error_message']	= $post['Message'];
					$this->gateway_order_update($resp, $this->order('order_id'), NULL);
					echo "StatusCode=30&Message=".$post['Message'];	
					exit; 
				}
				else
				{
					...
					$this->gateway_order_update($resp, $this->order('order_id'), NULL);
					echo "StatusCode=30&Message=". $this->lang("cardsave_server_no_message"); 
					exit; 
				}
		...
		$str1 = $post["HashDigest"];
		$hashcode = $this->createhash($post);
		if ($hashcode != $str1) 
		{
			$resp['error_message']	= $this->lang("cardsave_server_hashes_did_not_match"); 
			$this->gateway_order_update($resp, $this->order('order_id'), NULL);
			echo "StatusCode=30&Message=". $this->lang("cardsave_server_hashes_did_not_match"); 
			exit;
		}
	...
	public function callback($post)
	{
		...
		if ($sha == $hash) 
		{ 
			$this->gateway_order_update(NULL, $this->order('order_id'), $this->order('return'));
		} 
		else 
		{ 
			$resp['authorized']	 	= FALSE; 
