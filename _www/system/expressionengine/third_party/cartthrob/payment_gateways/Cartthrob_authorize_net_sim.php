...
class Cartthrob_authorize_net_sim extends Cartthrob_payment_gateway
{
	...
	public function process_payment($credit_card_number)
	{
		...
		{
			$header = $this->parse_template($this->fetch_template($this->plugin_settings('x_header_html_payment_form'))); 
 			$post_array["x_header_html_payment_form"]    = substr($header, 0, 254);
		}
		if ($this->plugin_settings('x_footer_html_payment_form'))
		{
			$footer = $this->parse_template($this->fetch_template($this->plugin_settings('x_footer_html_payment_form'))); 
 	...
 	function extload($post)
	{
		...
		$this->relaunch_cart_snapshot($post['x_po_num']);
		...
		if ($this->plugin_settings('hash_value'))
		{
			...
			if ($hash != $post['x_MD5_Hash'])
			{
				...
				$this->gateway_order_update($auth, $this->order('entry_id'), NULL, TRUE);
				echo $this->parse_template($this->fetch_template($this->order('return'))); 
			}
		}
		$purchased_items_status = NULL; 
		$order_status = NULL; 
		...
		switch ($post["x_response_code"])
		{
 			case "1":
				...
				$this->update_order_by_id($order_id, $order_data); 
				break;
			case "2":
		...
		$this->gateway_order_update($auth, $this->order('entry_id'), NULL, $send_email = TRUE);
		echo $this->parse_template($this->fetch_template($this->order('return'))); 
		exit; 	
