...
class Coupon_code_model extends CI_Model
{
	...
	public function validate_coupon_code($coupon_code)
	{
		...
		if ( ! $data['metadata']['valid'])
		{
			...
			{
				$msg = sprintf($this->lang->line('coupon_global_limit_msg'), $this->store->config('global_coupon_limit'));
			}
	...
	public function get_coupon_code_data($coupon_code)
	{
		...
		if ($this->store->config('coupon_code_channel') && $this->store->config('coupon_code_type'))
		{
			$coupon_field = 't.title';

			if ($this->store->config('coupon_code_field') && $this->store->config('coupon_code_field') != 'title')
			{
				$coupon_field = 'd.field_id_'.$this->store->config('coupon_code_field');
			}
			
			$this->db->from('channel_titles t')
					->join('channel_data d', 't.entry_id = d.entry_id')
					->where('t.channel_id', $this->store->config('coupon_code_channel'))
			...
			if ($query->num_rows())
			{
				$data = _unserialize($query->row('field_id_'.$this->store->config('coupon_code_type')), TRUE);
				...
				$data['metadata']['invalid'] = FALSE;
				$data['metadata']['global_limit'] = ($this->store->config('global_coupon_limit') > 1 && count($this->cartthrob->cart->coupon_codes()) >= $this->store->config('global_coupon_limit'));
	...
	{
		foreach ($this->cart->coupon_codes() as $coupon_code)
		{
			...
			if ($entry_id && $this->store->config('coupon_code_type'))
			{
				unset($data['metadata']);
				...
				$this->db->update('channel_data', array('field_id_'.$this->store->config('coupon_code_type') => base64_encode(serialize($data))), array('entry_id' => $entry_id));
			}
		}
