...
class Discount_model extends CI_Model
{
	...
	public function process_discounts()
	{
		foreach ($this->get_valid_discounts() as $entry_id => $data)
		{
			...
			if ($this->store->config('discount_type'))
			{
				...
				$this->db->update('channel_data', array('field_id_'.$this->store->config('discount_type') => base64_encode(serialize($data))), array('entry_id' => $entry_id));
			}
		}
	}
	...
	public function get_valid_discounts()
	{
		if (is_null($this->discount_data))
		{
			...
			if ($this->store->config('discount_channel') && $this->store->config('discount_type'))
			{
				$filter = array(
					'channel_titles.status !=' => 'closed',
					'channel_titles.channel_id' => $this->store->config('discount_channel'),
				...
				foreach ($entries as &$entry)
				{
					...
					$data = _unserialize(element('field_id_'.$this->store->config('discount_type'), $entry), TRUE);
