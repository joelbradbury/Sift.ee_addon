...
class Cartthrob_session
{
	...
	/**
	 * @var Cartthrob_core The CartThrob app instance
	 */
	protected $core;
	...
	public function __construct($params = array())
	{
		...
		{
			$this->core =& $this->EE->cartthrob;
		}
		else
		{
			//we need this because when this library is loaded, the core CT app has not yet been attached to $this->EE
			if ( ! isset($params['core']))
			{
				//backwards compat
				if ( ! isset($params[0]))
				{
					show_error('No core provided to cartthrob_session library');
				}
			...
			$this->EE->cartthrob =& $this->core;
		}
		
		if ( ! $this->core->store->config('session_use_fingerprint') || (isset($params['use_fingerprint']) && ! $params['use_fingerprint']))
		{
		...
		if ($this->core->store->config('session_expire'))
		{
			$this->expires = $this->core->store->config('session_expire');
		}
		...
		if ($this->session_id)
		{
			...
			$query = $this->EE->db->order_by('expires', 'desc')->limit(1)->get('cartthrob_sessions');			
		...
		if ( ! $this->EE->cartthrob->store->config('garbage_collection_cron') && rand(1, 100) <= $this->garbage_collection_probability)
		{
			$this->garbage_collection();
		}
	}
	...
	{
		switch($this->core->store->config('session_fingerprint_method'))
		{
