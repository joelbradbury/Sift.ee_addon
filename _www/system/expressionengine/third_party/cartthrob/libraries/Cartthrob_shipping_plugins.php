...
class Cartthrob_shipping_plugins
{
	...
	{
		if ($this->cart->customer_info('shipping_'.$location))
		{
			return $this->cart->customer_info('shipping_'.$location); 
		}
		elseif ($this->cart->customer_info($location))
		{
			return $this->cart->customer_info($location); 
		}
		else
		{
			// looking through custom data for this information.
			if ($this->cart->custom_data($location))
			{
				return $this->cart->custom_data($location); 
			}
			else
			{
				...
				{
					return $this->store->config('default_location', $location);
				}
			}
		}
	}
