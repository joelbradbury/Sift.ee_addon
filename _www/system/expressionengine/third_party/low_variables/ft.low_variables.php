...
class Low_variables_ft extends EE_Fieldtype {
	...
	*/
	...
	public function display_field($data = '', $cell = FALSE)
	{
		...
		$query = $this->EE->db->select('ee.variable_name, low.variable_label, vg.group_label')
		       ->from('global_variables ee')
		       ->join('low_variables low', 'ee.variable_id = low.variable_id')
		       ->join('low_variable_groups vg', 'low.group_id = vg.group_id')
		...
		{
			$vars[$row['group_label']][$row['variable_name']] = $row['variable_label'];
		}
