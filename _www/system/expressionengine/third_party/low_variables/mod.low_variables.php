...
class Low_variables extends Low_variables_base
{
		...
		if ($var && array_key_exists($var, $this->vars))
		{
			...
			if (($OBJ = $this->_get_type_object($type)) !== FALSE)
			{
				if (method_exists($OBJ, 'display_output'))
				{
					...
					$tagdata = $OBJ->display_output($tagdata, $the_var);
				}
				else
				{
	...
	private function _get_var_param()
	{
		...
		{
			$site_name = substr($var, 0, $pos);
			$var       = substr($var, $pos + 1);

			// If site name is not current site, fetch its id from cache or DB
			if ($site_name != $this->EE->config->item('site_short_name'))
			{
				...
				// If site name is known, use its id
				if (array_key_exists($site_name, $sites))
				{
