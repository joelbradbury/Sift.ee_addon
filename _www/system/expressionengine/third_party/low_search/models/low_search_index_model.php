...
class Low_search_index_model extends Low_search_model {
	...
	public function build($collection = array(), $entry = array())
	{
		...
		{
			$settings = Low_search_base::decode($settings, FALSE);
		}
		...
		{
			// --------------------------------------
			// Initiate string to add to the index
			// --------------------------------------
			...
			// --------------------------------------
			// String should still have length after cleaning
			// --------------------------------------

			if (strlen($str = low_clean_string($str)))
			{
				// Create pipe-separated weighted string
				$text[] = trim(self::WEIGHT_SEPARATOR.str_repeat($str.self::WEIGHT_SEPARATOR, $field_weight));
			}
		}
		...
		{
			$this->collections[$collection_id] = isset($collection['site_id']) ? $collection['site_id'] : $this->site_id;
		}
